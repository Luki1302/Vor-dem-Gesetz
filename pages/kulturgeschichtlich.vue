  ...
  content.querySelectorAll(':scope > section').forEach((section) => {
    ...
    function flushMedia(captionAside) {
      ...
      const hasWrapper = pendingMedia.some(el => {
        const cls = el.className || ''
        return cls.includes('media-gallery') || cls.includes('media-trio') || cls.includes('media-duo') || cls.includes('media-2x1')
      })
      const wrapperType = pendingMedia.find(el => (el.className || '').includes('media-2x1')) ? '2x1' :
        pendingMedia.find(el => (el.className || '').includes('media-duo')) ? 'duo' :
        pendingMedia.find(el => (el.className || '').includes('media-gallery')) ? 'gallery' : ''
      ...
      pendingMedia.forEach((el) => {
        const cls = el.className || ''
        if (cls.includes('media-gallery') || cls.includes('media-trio') || cls.includes('media-duo') || cls.includes('media-2x1')) {
          ...
          else if (img) items.push({ type: 'img', src: img.src || img.dataset?.src || '', orient: itemOrient })
        }
      })
...
function updateTocActive() {
  ...
  const sections = toc.map(t => ({ id: t.id, el: document.getElementById(t.id) })).filter(s => s.el)
