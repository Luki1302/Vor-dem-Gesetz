...
const [feb, marz, apr, mai, jun] = await Promise.all([
  queryContent('/gestalterisch/prozess/februar').findOne().catch(() => null),
  queryContent('/gestalterisch/prozess/maerz').findOne().catch(() => null),
  queryContent('/gestalterisch/prozess/april').findOne().catch(() => null),
  queryContent('/gestalterisch/prozess/mai').findOne().catch(() => null),
  queryContent('/gestalterisch/prozess/juni').findOne().catch(() => null),
])
...
const monthToc = {
  februar: ['Kick-off', 'Experimente', 'Workshop'],
  maerz:   ['Experimente', 'Interviews', 'Recherche'],
  april:   ['Recherche', 'Experimente'],
  mai:     ['Eye Tracking', 'Konzeptsuche', 'Das neue Konzept', 'Erste gestalterische Ansätze', 'Installation', 'Technischer Aufbau'],
  juni:    ['UI-Design', 'Animation', 'Installation & DMX'],
}
...
function getOrientation(el) {
  ...
  if (fig?.className?.includes('figure-quer') || vid?.className?.includes('video-quer') || img?.classList?.contains('img-quer')) return 'landscape'
...
function showSlot(id) {
  ...
  if (activeSlotId) {
    ...
    if (prev) {
      prev.querySelectorAll('video').forEach((v) => { v.pause(); v.removeAttribute('src') })
      prev.querySelectorAll('img').forEach((img) => { img.removeAttribute('src') })
      prev.remove()
    }
  ...
  if (data.isBulk && data.items.length >= 7 && data.items.length <= 14 && data.items.length !== 12) slot.classList.add('bulk-4col')
  ...
  if (data.isUnequal) {
    ...
    const loaders2 = data.items.map(item => {
      if (item.type === 'video') {
        ...
        const img = new Image()
        img.src = item.src
        return img.decode().then(() => img.naturalWidth / img.naturalHeight).catch(() => 1)
      }
    })
  ...
  data.items.forEach((item, idx) => {
    ...
    if (!isBulk3 && !isSplit3 && !slot.classList.contains('bulk-5') && el) {
      const widthMap = { landscape: '100%', full: '85%', portrait: '75%', small: '60%', mini: '50%', xs: '40%' }
      const itemOrient = data.items[idx]?.orient || data.orient
      const w = widthMap[itemOrient] || widthMap[data.orient] || '100%'
      if (el.tagName === 'FIGURE') { el.style.maxWidth = w; el.querySelector('img').style.width = '100%' }
...
function isMediaElement(el) {
  ...
  const cls = el.className || ''
  if (cls.includes('moodboard') || cls.includes('media-') || cls.includes('img-duo') || cls.includes('img-trio')) return true
    ...
    content.querySelectorAll(':scope > section').forEach((section) => {
      ...
      function flushMedia(captionAside) {
        ...
        const hasWrapper = pendingMedia.some(el => {
          const cls = el.className || ''
          return cls.includes('media-gallery') || cls.includes('media-trio') || cls.includes('media-duo') || cls.includes('media-2x1') || cls.includes('media-unequal')
        })
        ...
        const isSplitWide = pendingMedia.length === 2 && !isBulk &&
          ((o1 === 'landscape' && o2 === 'portrait') || (o1 === 'portrait' && o2 === 'landscape'))

        const items = []
        pendingMedia.forEach((el) => {
          const cls = el.className || ''
          if (cls.includes('media-gallery') || cls.includes('media-trio') || cls.includes('media-duo') || cls.includes('media-2x1') || cls.includes('media-unequal')) {
            ...
            else if (img) items.push({ type: 'img', src: img.src || img.dataset.src || '', orient: itemOrient, darkSrc: img.dataset.dark || '', lightSrc: img.dataset.light || '' })
        ...
        slotStore.set(mediaId, {
          items,
          caption: captionAside ? captionAside.textContent.trim() : null,
          isBulk,
          isSplitWide,
          allLandscape,
          hasWrapper,
        ...
        marker.addEventListener('click', (e) => { e.preventDefault(); showSlot(mediaId) })

        if (lastTextEl && lastTextEl.parentNode === textCol) {
          lastTextEl.appendChild(marker)
        } else {
          textCol.appendChild(marker)
        }
  ...
  window.addEventListener('scroll', () => {
    ...
    scrollRaf = requestAnimationFrame(() => {
      ...
      pairs.forEach((p, i) => {
        ...
        if (rect.top <= viewMid + 100 && dist < closestDist) {
          closestDist = dist
          closest = i
        }
      })
      showSlot(pairs[closest].slotId)
      updateTocActive()
    })
  }, { passive: true })
...
<template>
  <div>
    ...
    <div class="pz-raw-content" style="position: fixed; top: 0; left: 0; width: 0; height: 0; overflow: hidden; pointer-events: none">
